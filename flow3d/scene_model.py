...
class SceneModel(nn.Module):
    def __init__(
        ...
        self.fg = fg_params
        self.bg = bg_params
        # 运动基，一组随时间变化的 SE(3) 变换
        self.motion_bases = motion_bases
        # 背景场景尺度，如果背景高斯不存在，则默认为 1.0
        ...
        # 可训练模块，允许模型微调相机轨迹以纠正 SLAM 误差，注意这里和外参不同，外参是固定的，这里是可以训练的
        self.camera_poses = camera_poses

        # 当前 2D 高斯中心，用于自适应高斯控制
    ...
    def compute_transforms(
        ...
        coefs = self.fg.get_coefs()  # (G, K) 前景高斯关于运动基的权重
        if inds is not None:
            coefs = coefs[inds]
    ...
    def compute_poses_fg(
        ...
        """
        means = self.fg.params["means"]  # (G, 3) 前景高斯在参考帧的坐标
        quats = self.fg.get_quats()  # (G, 4) 前景高斯在参考帧的朝向
        if inds is not None:
            means = means[inds]
            quats = quats[inds]
        if ts is not None:
            # 得到每个高斯针对每个帧的变换
            transfms = self.compute_transforms(ts, inds)  # (nums_track_points, B, 3, 4)
            # 把这个变换应用到初始的高斯坐标上，得到每个高斯针对每个帧的坐标 (nums_track_points, B, 3)
            means = torch.einsum(
                "pnij,pj->pni",
                transfms,
                F.pad(means, (0, 1), value=1.0),
            )
            # 把这个变换应用到初始的高斯朝向上，得到每个高斯针对每个帧的朝向 (nums_track_points, B, 4)
    ...
    def init_from_state_dict(state_dict, prefix=""):
        ...
        return SceneModel(Ks, w2cs, fg, motion_bases, camera_poses, bg)

    def render(   # 这个函数是核心渲染函数，负责将高斯球渲染到图像上
        self,
        ...
        curr_w2cs = w2cs  # 当前相机位姿

        # Ensure local is always defined to avoid UnboundLocalError when target_ts is provided
        ...
        if means is None or quats is None:
            means, quats = pose_fnc(torch.tensor([t], device=device) if t is not None else None)
            means = means[:, 0]
            quats = quats[:, 0]
        ...
        # 3. 使用 2DGS 渲染，或者不使用
        ## 输入是成千上万高斯球，输出是 2D 图片
        if self.use_2dgs:   # 不使用
            ...
            (render_colors, render_alphas, render_normals, surf_normals, _, _, info) = outputs
        else:
