...
class Trainer:
    def __init__(
        ...
        self.global_step = 0
        self.epoch = 0
        if not hasattr(self, "viewer_render_uses_timestep"):
            self.viewer_render_uses_timestep = True
    ...
    def init_from_checkpoint(
        ...
        # 兼容 PyTorch 2.6 的 weights_only 默认安全限制
        ckpt = torch.load(path, weights_only=False)
        state_dict = ckpt["model"]
        model = SceneModel.init_from_state_dict(state_dict)
        model = model.to(device)
        print(f"use_2dgs: {use_2dgs}")
    ...
    def render_fn(self, camera_state: CameraState, tab_state_or_img_wh):
        W, H = self._resolve_viewer_img_wh(camera_state, tab_state_or_img_wh)
        ...
        t = self._viewer_current_timestep(default=0) if self.viewer_render_uses_timestep else 0
        self.model.training = False
        img = self.model.render(t, w2c[None], K[None], (W, H))["img"][0]
        return (img.cpu().numpy() * 255.0).astype(np.uint8)

    def _viewer_current_timestep(self, default: int = 0) -> int:
        try:
            viewer = getattr(self, "viewer", None)
            if viewer is None:
                return int(default)
            canonical = getattr(viewer, "_canonical_checkbox", None)
            if canonical is not None and bool(getattr(canonical, "value", False)):
                return 0
            playback_guis = getattr(viewer, "_playback_guis", None)
            if not playback_guis:
                return int(default)
            timestep = int(getattr(playback_guis[0], "value", default))
            return int(np.clip(timestep, 0, max(self.model.num_frames - 1, 0)))
        except Exception:
            return int(default)

    def _resolve_viewer_img_wh(self, camera_state: CameraState, tab_state_or_img_wh):
        if isinstance(tab_state_or_img_wh, (tuple, list)) and len(tab_state_or_img_wh) == 2:
            return int(tab_state_or_img_wh[0]), int(tab_state_or_img_wh[1])

        ts = tab_state_or_img_wh
        W = getattr(ts, "viewer_width", None)
        H = getattr(ts, "viewer_height", None)
        if W is None or H is None:
            W = getattr(ts, "width", None)
            H = getattr(ts, "height", None)
        if W is None or H is None:
            res = getattr(ts, "resolution", None)
            if isinstance(res, (tuple, list)) and len(res) == 2:
                W, H = res

        if W is None or H is None:
            max_res = int(getattr(ts, "viewer_res", 1024))
            aspect = max(float(getattr(camera_state, "aspect", 1.0)), 1e-6)
            if aspect >= 1.0:
                W = max_res
    ...
    def train_step(self, batch):
        if self.viewer is not None:
            ...
            # 兼容不同 nerfview 版本：state/lock 可能不存在或类型不同
            try:
                viewer_state = getattr(self.viewer, "state", None)
                status = getattr(viewer_state, "status", None)
                while status == "paused":
                    time.sleep(0.1)
                    status = getattr(viewer_state, "status", None)
            except Exception:
                pass
            viewer_lock = getattr(self.viewer, "lock", None)
            if viewer_lock is not None:
                try:
                    viewer_lock.acquire()
                except Exception:
                    pass
        ...
        if self.viewer is not None:
            ...
            if viewer_lock is not None:
                try:
                    ...
                    pass
            viewer_state = getattr(self.viewer, "state", None)
            if viewer_state is not None and hasattr(viewer_state, "num_train_rays_per_sec"):
                try:
    ...
    def compute_losses(self, batch):
        ...
        print(f"当前场景的高斯数量为: {means.shape[0]}")
        device = means.device
    ...
    def _prepare_control_step(self) -> bool:
        ...
        for _current_xys, _current_radii, _current_img_wh in zip(
            ...
            for c in range(C):
                sel_c = sel[c]
                # 将除高斯维(当前为 dim=0)外的其他维度做 any 归约
                if sel_c.ndim > 1:
                    for d in range(sel_c.ndim - 1, 0, -1):
                        sel_c = sel_c.any(dim=d)
                gidcs = torch.where(sel_c)[0]
                if gidcs.numel() == 0:
                    continue
                selected_grads = xys_grad[c, gidcs]  # (K, 2)
                grad_norms = selected_grads.norm(dim=-1)  # (K,)
                self.running_stats["xys_grad_norm_acc"].index_add_(0, gidcs, grad_norms)
                ...
                if radii_c.ndim > 1:
                    for d in range(radii_c.ndim - 1, 0, -1):
                        radii_c = radii_c.max(dim=d).values
                curr = radii_c[gidcs] / max(_current_img_wh)
                max_radii = torch.maximum(
                    self.running_stats["max_radii"].index_select(0, gidcs), curr
                )
