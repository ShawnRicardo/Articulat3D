...
def get_tracks_3d_for_query_frame(
    ...
    # 利用 Bootstapir 提供的 2D 坐标 (u, v) ，去每一帧深度图里查表，得到这个点在每一帧的深度 d
    ...
    # 
    tracks_3d = (
        torch.einsum(   # einsum 爱因斯坦求和符号，是一个超灵活的矩阵乘法
            "nij,npj->npi",
            inv_Ks,
            F.pad(tracks_2d, (0, 1), value=1.0),    # 用在最后一维，给 (x,y) 补一个 1 变成 (x, y, 1) (0, 1)表示前面补 0 个，后面补 1 个
        )   # 这里是 inv_Ks @ (x, y, 1) = (x', y', 1)
        * track_depths[..., None]   # None 表示在最后一维添加一个维度。实际上就是再乘以深度得到相机坐标系下的 3D 点坐标
    )
    # 将这个坐标乘以 c2ws 得到世界坐标系下的 3D 点坐标
